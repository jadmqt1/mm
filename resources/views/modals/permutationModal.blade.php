<div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <form class="p-4 md:p-5 relative bg-white rounded-lg shadow dark:bg-gray-700" method="POST"  action="{{route('permutations.store')}}" enctype="multipart/form-data">
                @csrf
                    <!-- Modal header -->
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-6 gap-6">
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <input type="date"  name="date" id="date" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="date" required="">
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                <label for="formateur_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">formateur</label>
                                <select name="formateur_id" id="formateur_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                                    <option value="" disabled></option>
                                    @foreach ($formateurs as $formateur)
                                    @if (strtotime($formateur->date_recrutement) < time())
                                        
                                    
                                    <option value="{{$formateur->id}}" selected>{{$formateur->nom}} {{$formateur->prenom}}</option>
                                    @endif
                                    @endforeach
                                        </select>    
                            </div>
